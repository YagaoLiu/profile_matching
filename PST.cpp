...
PropertySuffixTree::stNode::stNode(PropertySuffixTree::position const &begin, PropertySuffixTree::position const &end): suf_link(NULL), children(), begin(begin), end(end), PatternID(-1) {}
...
void PropertySuffixTree::EndingNodeMark(vector<Path> &V) const{
	for( auto v:V ){
		PropertySuffixTree:: Locus traversal(root, v.start, v.end+1);
		traversal.fast_forward();
		traversal.node->PatternID = v.PatternID;
	}
}
